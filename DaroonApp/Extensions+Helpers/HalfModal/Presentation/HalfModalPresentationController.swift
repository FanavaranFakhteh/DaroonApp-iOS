...
//
//zzmasoud: I didn't really want half page - so some codes changed by me.
    ...
    @objc func onPan(pan: UIPanGestureRecognizer) -> Void {
        ...
        case .changed:
            ...
            if direction > 0 {
                switch state {
                case .normal:
                    presentedView!.frame.origin.y = endPoint.y + containerView!.bounds.height - containerView!.bounds.height/3
                case .adjustedOnce:
...
//                    changeScale(to: .normal)
    ...
    func changeScale(to state: ModalScaleState) {
        if let presentedView = presentedView, let containerView = self.containerView {
            UIView.animate(withDuration: 0.8, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: .curveEaseInOut, animations: { () -> Void in
                ...
                let halfFrame = CGRect(origin: CGPoint(x: 0, y: containerView.bounds.height - containerView.bounds.height/3),
                                       size: CGSize(width: containerFrame.width, height: containerView.bounds.height - containerView.bounds.height/3))
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        return CGRect(x: 0, y: containerView!.bounds.height - containerView!.bounds.height/3 , width: containerView!.bounds.width, height: containerView!.bounds.height / 3)
    }
